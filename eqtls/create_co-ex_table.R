...
for (meta_file in list.files(co_ex_tables_path, ".*meta.*", full.names = T)) {
  ...
  all_genes <- all_genes[all_genes!="significance_threshold"]
  all_genes <- c("significance_threshold", all_genes)
  ...
  co_ex_table_out[, "UT meta p-value"] <- meta_table[rownames(co_ex_table_out),"UT"]
  ...
  for (pathogen in pathogens) {
    for (timepoint in timepoints) {
      condition <- paste0("X", timepoint, pathogen)
      if (!condition %in% colnames(meta_table)) { next() }
      co_ex_table_out[, paste0(timepoint, pathogen, " meta p")] <- meta_table[rownames(co_ex_table_out),condition]
