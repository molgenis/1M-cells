############################################################################################################################
# Authors: Roy Oelen
# Name: 1M_create_features_reclassified_T.R
# Function: create the mean expression per cell type and condition files used for eQTL mapping, specifically for the T cells
############################################################################################################################
...
object_dir <- paste('/groups/umcg-bios/', read_partition, '/projects/1M_cells_scRNAseq/ongoing/seurat_preprocess_samples/objects/', sep = '')
...
v2_features_demux_dir <- paste(features_dir, 'v2_sct_mqc_demux_hires_20201029_reclassified_T/', sep = '')
v3_features_demux_dir <- paste(features_dir, 'v3_sct_mqc_demux_hires_20201106_reclassified_T/', sep = '')

# location of the T annotations
t_classification_loc <- paste('/groups/umcg-bios/', read_partition, '/projects/1M_cells_scRNAseq/ongoing/cell-type-classifying/seurat_multimodal/classifications/', sep = '')
...
t_classification_v2 <- read.table(t_classification_loc_v2, sep = '\t', header = T, row.names = 1)
v2 <- AddMetaData(v2, t_classification_v2['clustered.celltype.l2.t'])
